...
# Pattern: zyvo-{module}-{descriptive-keywords}.png
set -euo pipefail
...
if [[ -d e-commerce ]]; then
  ...
  # Move renamed files to virtual-store/
...
# Seed appointment scheduling from HR time until dedicated screenshots are added
if [[ ! -f appointment/zyvo-appointment-scheduling-calendar.png ]] && [[ -f employee/zyvo-hr-employee-time-attendance.png ]]; then
  cp employee/zyvo-hr-employee-time-attendance.png appointment/zyvo-appointment-scheduling-calendar.png
  git add appointment/zyvo-appointment-scheduling-calendar.png 2>/dev/null || true
fi
if [[ ! -f appointment/zyvo-appointment-online-booking.png ]] && [[ -f employee/zyvo-hr-employee-time-attendance.png ]]; then
  cp employee/zyvo-hr-employee-time-attendance.png appointment/zyvo-appointment-online-booking.png
  git add appointment/zyvo-appointment-online-booking.png 2>/dev/null || true
fi

echo "==> Virtual store mobile (seed from desktop if missing)"
if [[ ! -f virtual-store/mobile/zyvo-virtual-store-mobile-catalog.png ]] && [[ -f virtual-store/zyvo-virtual-store-public-orders-catalog.png ]]; then
  mkdir -p virtual-store/mobile
  cp virtual-store/zyvo-virtual-store-public-orders-catalog.png virtual-store/mobile/zyvo-virtual-store-mobile-catalog.png
  git add virtual-store/mobile/zyvo-virtual-store-mobile-catalog.png 2>/dev/null || true
fi
if [[ ! -f virtual-store/mobile/zyvo-virtual-store-mobile-shopping-app.png ]] && [[ -f virtual-store/zyvo-virtual-store-public-orders-catalog.png ]]; then
  cp virtual-store/zyvo-virtual-store-public-orders-catalog.png virtual-store/mobile/zyvo-virtual-store-mobile-shopping-app.png
  git add virtual-store/mobile/zyvo-virtual-store-mobile-shopping-app.png 2>/dev/null || true
fi
